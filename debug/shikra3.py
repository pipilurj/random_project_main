...
from mllm.utils.box_utils import giou_loss
...
def loc_to_coords(loc, num_locations):
    ...
    xc, yc = token_num - math.floor((token_num - 0.1) / num_locations)*num_locations, math.ceil(token_num / num_locations)
...
class ShikraLlamaForCausalLM3(LlamaForCausalLM):
    ...
    def forward(
        ...
        # loc_tokens = self.tokenizer.batch_decode(loc_token_ids)
        # offsets = []
        # for box, loc in zip(torch.cat(boxes_seq), loc_tokens):
        #     offset = box_loc_to_offsets(box, loc, math.sqrt(len(self.loc_tokens_ids)))
        #     offsets.append(offset)
        # offsets = torch.tensor(offsets).to(input_ids.device)
        outputs = self.model(
            input_ids=input_ids,
