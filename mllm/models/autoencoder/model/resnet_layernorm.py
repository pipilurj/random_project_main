...
class ResNet(nn.Module):
    def __init__(self, downblock, upblock, num_layers, n_classes, norm_enc="ln", norm_dec="ln"):
        ...
        self.dlayer4 = self._make_downlayer(downblock, 32, num_layers[3],
                                            stride=2, norm=norm_enc)
        # self.dlayer5 = self._make_downlayer(downblock, 32, num_layers[3],
        #                                     stride=2)
        # self.conv_enc = nn.Conv2d(64, 64, kernel_size=3, stride=1, padding=1,
        #                        bias=False)
        self.linear_enc = nn.Linear(32*2*8*8, 4096)
        self.linear_dec = nn.Linear(4096, 32*2*8*8)
        self.uplayer1 = self._make_up_block(upblock, 128, 1, stride=2, norm=norm_dec)
        self.uplayer2 = self._make_up_block(upblock, 64, 2, stride=2, norm=norm_dec)
        self.uplayer3 = self._make_up_block(upblock, 32, num_layers[2], stride=2, norm=norm_dec)
        self.uplayer4 = self._make_up_block(upblock, 16, num_layers[1], stride=2, norm=norm_dec)
        ...
        self.uplayer_top = DeconvBottleneck(self.in_channels, 16, 1, 2, upsample)
    ...
    def forward(self, x, return_embedding = False, noise_scale=0.):
        x_size = x.size()
        repr = self.encode(x)
        repr_noise = noise_scale * torch.randn_like(repr) + repr
        reconstruction = self.decode(repr_noise, x_size)
        if return_embedding:
