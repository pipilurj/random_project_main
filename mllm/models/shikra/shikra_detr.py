...
class ShikraLlamaForCausalLMDetr(LlamaForCausalLM):
    config_class = ShikraConfig
    ...
    def init_bbox_head(self, config):
        self.bbox_head = MLP(self.config.mm_hidden_size, self.config.mm_hidden_size, 4, 3)
        self.llm2detr = nn.Linear(self.config.hidden_size, self.config.mm_hidden_size)
        self.detr_decoder = Transformer(d_model=self.config.mm_hidden_size)
    ...
    def decode_offsets_from_hidden(self, loc_embeddings):
        hs = self.detr_decoder(loc_embeddings, self.model.image_features)
        offset_pred = self.bbox_head(hs)
        return offset_pred
    ...
    def decode_box_from_token(self, input_ids, attention_mask, images, batchify=True):
        ...
        offsets = self.decode_offsets_from_hidden(loc_embeddings)
    ...
    def forward(
        ...
        if self.training:
            ...
            loc_embeddings = [self.llm2detr(hidden_states[i,id,:]) for i, id in enumerate(pos_of_loc_tokens)]
            offset_pred = self.decode_offsets_from_hidden(loc_embeddings)#.sigmoid() # x, y, w, h
            ...
            if torch.distributed.is_initialized():
                if torch.distributed.get_rank() == 0:
                    for i, (pred, gt) in enumerate(zip(box_loc_pred_reformat.data, boxes_seq.data)):
