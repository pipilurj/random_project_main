...
def plot_images(real, gen, noise, save_path="", imgid=0):
    import matplotlib.pyplot as plt
...
class ShikraLlamaForCausalLMMask(LlamaForCausalLM):
    ...
    def init_autoencoder(self, config):
        ...
        self.encoding_mapping = nn.Linear(4096, 4096)
    ...
    # def mask_loss(self, pred, gt):
    #     # mse_loss = F.mse_loss(pred, gt)
    #     mse_loss = F.l1_loss(pred, gt)
    #     return mse_loss
    def mask_loss(self, reconstruction, mask_pixel, pred_embed, gt_embed):
        ...
        l1_loss = F.l1_loss(pred_embed, gt_embed)
        return loss_reconstruction, l1_loss
    ...
    def decode_box_from_token(self, input_ids, attention_mask, images, batchify=True):
        outputs = self.model(
            input_ids=input_ids,
        ...
        if len(loc_token_ids) == 0:
            return [torch.tensor([[0., 0., 0., 0.]]).to(hidden_states.device)] * len(input_ids)
        center_coords = torch.tensor([self.loc_id_to_coord[id.item()] for id in loc_token_ids]).to(input_ids.device)
        loc_embeddings = [self.llm2detr(hidden_states[i, id, :]) for i, id in enumerate(pos_of_loc_tokens)]
        hs = self.detr_decoder(loc_embeddings, self.model.image_features)
        offsets = self.bbox_head(hs)
        bbox_pred = coord_offsets_to_bbox(center_coords, offsets)
        if not batchify:
            return bbox_pred
        else:
            box_loc_pred_batched, start = [], 0
            for ids in pos_of_loc_tokens:
                if len(ids) > 0:
                    box_loc_pred_batched.append(bbox_pred[start:start + len(ids)].detach().clip(min=0, max=1))
                else:
                    box_loc_pred_batched.append(torch.tensor([[0., 0., 0., 0.]]).to(loc_embeddings.device))
                start = start + len(ids)
            return box_loc_pred_batched
    ...
    def forward(
        ...
        if self.training:
            ...
            loc_embeddings = self.encoding_mapping(hidden_states[pos_of_loc_tokens[0], pos_of_loc_tokens[1]])
            ...
            _, pretrained_embedding = self.autoencoder(masks_seq.unsqueeze(dim=1), return_embedding=True)
            ...
            recon_loss, l1_loss = self.mask_loss(mask_decoded, masks_seq, loc_embeddings, pretrained_embedding)
            mask_loss = recon_loss
            ...
            if torch.distributed.is_initialized():
                if torch.distributed.get_rank() == 0:
                    ...
                    pretrained_generation = self.autoencoder.generate(encoder_repr=pretrained_embedding)
                    ...
                    if hasattr(self, "imgid"):
                        ...
                        self.imgid=0
                    plot_images(masks_seq[0].to(torch.float32).squeeze().detach().cpu().numpy(), learnt_generation[0].to(torch.float32).squeeze().detach().cpu().numpy(), pretrained_generation[0].to(torch.float32).squeeze().detach().cpu().numpy(), save_path=self.img_dir, imgid = self.imgid)
