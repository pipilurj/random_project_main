...
#     boxes1, boxes2 = boxes1*224, boxes2*224
...
def loc_to_coords(loc, num_locations):
    ...
    token_num = float(match.group(1)) + 1
    xc, yc = token_num % num_locations, math.ceil(token_num / num_locations)
...
def box_loc_to_offsets(box, loc, num_locations):
    ...
    x1_off, y1_off, x2_off, x2_off = xc_coord - box[0], yc_coord - box[1], box[2]-xc_coord, box[3]-yc_coord
    return [x1_off, y1_off, x2_off, x2_off]
...
class ShikraLlamaForCausalLM2(LlamaForCausalLM):
    config_class = ShikraConfig
    ...
    def record_loc_token_id(self, tokenizer, num_loc_tokens):
        ...
        self.loc_tokens = [f"<bin_{i}>" for i in range(num_loc_tokens**2)]
    ...
    def forward(
        ...
        if self.training:
            ...
            box_loc_pred = self.bbox_head(loc_embeddings).sigmoid() # x, y, w, h
            # box_loc_pred_reformat = box_convert(box_loc_pred, "xywh", "xyxy")
            bbox_loss = self.box_reg_loss(box_loc_pred, torch.cat(boxes_seq))
            # box_loc_pred_reformat = xywh2xyxy(box_loc_pred)
            box_loc_pred_reformat = box_loc_pred
            loss += bbox_loss
            for i, (pred, gt, embed) in enumerate(zip(box_loc_pred_reformat.data, torch.cat(boxes_seq).data, loc_embeddings.data)):
    ...
    def prepare_inputs_for_generation(
            self, input_ids, past_key_values=None, attention_mask=None, inputs_embeds=None, boxes_seq=None, **kwargs
    ):
        if past_key_values:
