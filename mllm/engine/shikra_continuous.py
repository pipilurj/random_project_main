...
import numpy as np
class ShikraTrainerContinuous(ShikraTrainer):
    def prediction_step(
            self,
            model: nn.Module,
        ...
        with torch.inference_mode():
            with self.compute_loss_context_manager():
                ...
                generated_tokens, boxes_seq = self.model.generate(**gen_kwargs)
        boxes_seq = np.array(boxes_seq).reshape(-1, 4)
        ...
        return loss, {"pred_tokens":generated_tokens, "pred_box":boxes_seq}, {"gt_labels":labels, "gt_boxes":inputs["boxes_seq"]}
