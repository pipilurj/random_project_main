...
class ShikraTrainer2(ShikraTrainer):
    def prediction_step(
            self,
            model: nn.Module,
        ...
        with torch.inference_mode():
            with self.compute_loss_context_manager():
                ...
                generated_tokens = self.model.generate(**gen_kwargs)
        # generated_tokens = generated_results["sequences"]
        attention_mask = generated_tokens.ne(self.tokenizer.pad_token_id)
        with torch.no_grad():
            # output_repassed = self.model(generated_tokens, attention_mask=attention_mask, images=inputs["images"])
            box_loc_pred = self.model.decode_box_from_token(generated_tokens, attention_mask=attention_mask, images=inputs["images"])
        # last_hidden_state = output_repassed["last_hidden_state"]
        # pos_of_loc_tokens, loc_token_ids = self.model.get_loc_ids_pos(generated_tokens)
        # center_coords = torch.tensor([self.model.loc_id_to_coord[id.item()] for id in loc_token_ids]).to(generated_tokens.device)
        # loc_embeddings = torch.cat([last_hidden_state[i,id,:] for i, id in enumerate(pos_of_loc_tokens)])
        # # print(f"loc_embeddings")
        # box_loc_pred = self.model.bbox_head(loc_embeddings)# x, y, w, h
        # box_loc_pred = self.model.decode_box_from_pred(center_coords, box_loc_pred)
        # box_loc_pred_batched = []
        # start = 0
        # for ids in pos_of_loc_tokense
        #     if len(ids) >  0:
        #         box_loc_pred_batched.append(box_loc_pred[start:start+len(ids)].detach().clip(min=0, max=1))
        #     else:
        ...
        return loss, {"pred_tokens":generated_tokens, "pred_box":box_loc_pred}, {"gt_labels":labels, "gt_boxes":inputs["boxes_seq"]}
