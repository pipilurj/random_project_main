...
class SingleImageConvDatasetMixin:
    ...
    def __getitem__(self, index, debug_mode=False) -> Dict[str, Any]:
        ...
        raw_conv, _, boxes_seq, points_seq, masks_seq = self.process_target(raw_conv, target, multimage_mode=multimage_mode)
        ...
        # boxes_seq = list(itertools.chain(*boxes_seq))
        ret_dict.update({"boxes_seq": boxes_seq})
        ret_dict.update({"masks_seq": masks_seq})
        ret_dict.update({"points_seq": points_seq})
        return ret_dict
    ...
    def process_target(self, raw_conv: List[Dict[str, Any]], target: Dict[str, Any], multimage_mode=False):
        """
